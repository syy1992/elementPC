<template>
  <div class="app-container">
    <el-card class='mg-bot20 card '>
      <el-form :inline="true" :model="achievementObj" class="demo-form-inline min-width" label-position="top">
        ...
        <!--<el-form-item label="平台:">-->
          <!--<el-select v-model="achievementObj.platNo" placeholder="请选择" size="small" clearable>-->
            <!--<el-option-->
        ...
        <el-form-item>
          ...
          <el-button type="primary"  size="small" @click="onSubmit('channel')">查询渠道业绩</el-button>
        </el-form-item>
      </el-form>
    ...
    <!--<el-card class='mg-bot20 card '>-->
      <el-card  class='mg-bot20 card '>
        <div slot="header" class="clearfix mg-bot20">
          <span class='tit' style='line-height: 32px'><i class='iconfont icon-liebiao mg-right5'></i>产品销售动态</span>
        ...
        <chart  ref='chart1' :xAxisdata='xAxisdata' :seriesdata1='seriesdata1'  :seriesdata2='seriesdata2' v-if='showTab'></chart>
        <el-table  v-loading="listLoading"   :data="list" element-loading-text="Loading"   border  fit highlight-current-row :header-cell-style="{background:'#DFE6EC'}"  empty-text="暂无数据"  v-if='!showTab'>
          ...
          <el-table-column  label="操作" width="240" align='center' >
            <template slot-scope="scope">
              <el-button  type="text" size="small" @click='showProductDetail(scope.row,scope.$index)'>查看详情</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
      ...
      <div slot="header" class="clearfix">
        ...
        <span class='gray' style='line-height: 32px'>·数据为实时统计 ·包含渠道及下级业务员数据 ·按照支付时间统计已成交数据</span>
        <!--<el-button size='small' type="primary" class='fl-r'>下载报表</el-button>-->
        <el-button size='small' class='fl-r mg-right10' @click='showTab2=!showTab2'>切换图表</el-button>

      </div>
        <chart   ref='chart2' :xAxisdata='xAxisdata2' :seriesdata1='ydata1'  :seriesdata2='ydata2' v-if='showTab2'></chart>
        <el-table  v-loading="listLoading2"   :data="list2" element-loading-text="Loading"   border  fit
                   highlight-current-row :header-cell-style="{background:'#DFE6EC'}"  empty-text="暂无数据"  v-if='!showTab2'>
          <el-table-column v-for="(col,index) in cols2"  :prop="col.prop" :label="col.label" :key='index' align='center'  :width="col.width" :type='col.type'></el-table-column>
          <el-table-column  label="操作" width="240" align='center' >
            <template slot-scope="scope">
    ...
    methods: {
      ...
      getProductAchivement() {
        ...
        let params={
          ...
          "channelNo":this.achievementObj.channelNo, ///this.achievementObj.channelNo
          "salesmanNo":"",
          ...
          if (response.data.resCode === '0000') {
            ...
            this.list=response.data.data.report
               this.list.forEach((item,index)=>{
                 this.xAxisdata.push(item.productName)
                 this.seriesdata1.push(item.policyCount)
                 this.seriesdata2.push(item.premium)
              });
        this.$refs.chart1.initChart(this.xAxisdata,this.seriesdata1,this.seriesdata2)
      ...
      getChannelAchivement() {
        ...
        this.$http.post(url, params).then((response) => {
          if (response.data.resCode === '0000') {
            ...
            this.list2=response.data.data.report
...
//        })
      ...
      addChannel(){},
      onSubmit(params){
        params=='channel'? this.getChannelAchivement() : this.getProductAchivement()
//          if(params=='product'){
//            this.getChannelAchivement()
//          }else{
//
